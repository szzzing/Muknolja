...
<body>
	...
	<div class="container">
	<br><br><br><br><br><br>
		<img src="${ contextPath }/resources/uploadFiles/${p.thumbnail}" class="img-fluid" alt="..." id="thumbnail">
		...
		<div id="title" class="input-group input-group-lg">
		  ...
		  <input type="text" class="title form-control" aria-label="Sizing example input" aria-describedby="inputGroup-sizing-lg" value="${ p.partyTitle }"required>
		</div>
			...
			<div class="col">
				<div id="location" class="form-floating">
					...
					<select id="selectLocation"class="form-select" aria-label="Default select example">
						...
						<option value="서울" <c:if test="${ p.partyArea == '서울' }">selected</c:if>>서울</option>
						<option value="인천" <c:if test="${ p.partyArea == '인천' }">selected</c:if>>인천</option>
						<option value="대전" <c:if test="${ p.partyArea == '대전' }">selected</c:if>>대전</option>
						<option value="대구" <c:if test="${ p.partyArea == '대구' }">selected</c:if>>대구</option>
						<option value="광주" <c:if test="${ p.partyArea == '광주' }">selected</c:if>>광주</option>
						<option value="부산" <c:if test="${ p.partyArea == '부산' }">selected</c:if>>부산</option>
						<option value="울산" <c:if test="${ p.partyArea == '울산' }">selected</c:if>>울산</option>
						<option value="세종" <c:if test="${ p.partyArea == '세종' }">selected</c:if>>세종</option>
						<option value="경기" <c:if test="${ p.partyArea == '경기' }">selected</c:if>>경기</option>
						<option value="강원" <c:if test="${ p.partyArea == '강원' }">selected</c:if>>강원</option>
						<option value="충북" <c:if test="${ p.partyArea == '충북' }">selected</c:if>>충북</option>
						<option value="충남" <c:if test="${ p.partyArea == '충남' }">selected</c:if>>충남</option>
						<option value="경북" <c:if test="${ p.partyArea == '경북' }">selected</c:if>>경북</option>
						<option value="경남" <c:if test="${ p.partyArea == '경남' }">selected</c:if>>경남</option>
						<option value="전북" <c:if test="${ p.partyArea == '전북' }">selected</c:if>>전북</option>
						<option value="전남" <c:if test="${ p.partyArea == '전남' }">selected</c:if>>전남</option>
						<option value="제주" <c:if test="${ p.partyArea == '제주' }">selected</c:if>>제주</option>
					</select>
				...
				<div class="location2">
					...
					<ol class="numbered">
						<c:forEach items="${ courseList }" var="course" varStatus="status">
							<li class="numberLi" style="cursor: pointer">${ course }<input type="hidden" value="${ contentIdList[status.index] }" class="contentId2"></li>
						</c:forEach>
					</ol>
				</div>
		...
		<!-- 내용 -->
	<form action="${contextPath}/updateParty.pa" method="POST" enctype="multipart/form-data">
		<div class="content form-floating">
		  <textarea class="form-control" placeholder="Leave a comment here" id="floatingTextarea2" style="height: 400px; resize: none;" name="partyContent">${ p.partyContent }</textarea>
		  <label for="floatingTextarea2">내용</label>
		</div>
		
		<div class="d-grid gap-2">
			<button type="submit" class="writeButton mukButton" style="width: 100%;" id="writeButton">수정하기</button>
 		</div>
 		...
 		<input type="hidden" name="partyTitle" id="hiddenTitle" value="${ p.partyTitle }">
	...
	<script>
		...
		writeButton.addEventListener('click', function(){
			...
			console.log(hiddenCourse.value);
			console.log(hiddenCourse2.value);
		});
		...
		const locationDiv = document.getElementById('location1');
	   	...
	   	$(document).on('click', '.searchInfo', function(){
	   		...
	   		var li = '<li class="numberLi" style="cursor: pointer">' + this.querySelector('span').innerText + ContentId + '</li>';
...
//  	✔datepicker
		$(function(){
			$("#daterangepicker").daterangepicker({
			    ...
			    startDate: '${p.partyStartDate}',
			    endDate: '${p.partyEndDate}'
			});
